...
const UserDashboardHome = () => {
  ...
  // Calculate total allocated, total expense, and remaining budget
  const totalAllocatedBudget = upazilaData
    ? upazilaData.expenseCollections.reduce(
        (acc, item) => acc + item.allocatedBudget,
        0
      )
    : 0;

  const totalExpense = upazilaData
    ? upazilaData.expenseCollections.reduce(
        (acc, item) => acc + item.expenseBudget,
        0
      )
  ...
  const expensePercentage =
    totalAllocatedBudget > 0 ? (totalExpense / totalAllocatedBudget) * 100 : 0;
  const remainingPercentage =
    totalAllocatedBudget > 0
      ? (totalRemainingBudget / totalAllocatedBudget) * 100
      : 0;
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {upazilaData ? (
        ...
        <p className="text-center text-gray-500">
          No data found for the current upazila.
        </p>
      )}
